...
from utils.nx_graph import mst
...
plt.title('MST Spiral-STC', fontdict={'fontname':'Times New Roman', 'size': 20})
...
route = stc_planner.spiral_route(S, dummy_parent, mst(G))
trajectory = stc_planner.generate_cover_trajectory(S, mst(G))
...
stc_planner.H = stc_planner.generate_decomposed_graph(G, [S])
print(stc_planner.__get_travel_weights__(trajectory))

# plt.savefig('.figure_scripts/mst_spiral_stc.pdf')
plt.show()
